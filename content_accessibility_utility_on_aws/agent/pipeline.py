...
def _run_audit(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if multi_page:
            ...
            agent_pages = _run_agent_on_candidate_pages(work, options)
            publish_root = work
        else:
...
def _run_agent_on_candidate_pages(work_dir: str, options: Dict[str, Any]) -> int:
    """Run the browser-backed agent on multi-page docs, bounded to candidates.

    Only pages likely to have issues the static path cannot fix — those with
    interactive controls or images — are worth the per-page browser+agent cost.
    Capped by ``max_agent_pages``. Returns the number of pages the agent
    processed. Any failure (browser unavailable, model error) is non-fatal: the
    statically-remediated page is left as-is.
    """
    ...
    candidates = _candidate_pages(work_dir, cap)
    if not candidates:
        return 0
    ...
    logger.info("Agent processed %d/%d candidate pages", processed, len(candidates))
    return processed


def _candidate_pages(work_dir: str, cap: int) -> List[str]:
    """Select up to ``cap`` HTML pages worth the agent pass (interactive/image).

    A page is a candidate if it contains an interactive control or an image —
    the elements whose accessibility (focus, contrast, name-role-value, alt) the
    agent adds value on. Pure text/table pages are left to the static path.
    """
