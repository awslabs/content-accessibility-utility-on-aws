...
def display_usage_view(usage_data_path: Optional[str] = None) -> None:
    ...
    with col1:
        ...
        try:
            ...
            st.metric(
                "Session Start", "N/A", help="Time when the processing session started"
            )
    ...
    with col2:
        ...
        st.metric(
            "Processing Duration",
            duration,
            help="Duration of the processing session in HH:MM:SS format",
        )
    ...
    summary_tab, bda_tab, bedrock_tab, cost_tab = st.tabs(
        ["Summary", "BDA Usage", "Bedrock Usage", "Cost Analysis"]
    )
    ...
    with summary_tab:
        ...
        with col1:
            ...
            st.metric(
                "Documents Processed",
                usage_data.get("bda_usage", {}).get("total_documents_processed", 0),
            )
        ...
        with col2:
            ...
            st.metric(
                "Total API Calls",
                usage_data.get("bedrock_usage", {}).get("total_calls", 0),
            )
    ...
    with bda_tab:
        ...
        st.text(
            f"BDA Project ARN: {usage_data.get('bda_usage', {}).get('project_arn', 'N/A')}"
        )

        # Create BDA processing details table
        processing_details = usage_data.get("bda_usage", {}).get(
            "processing_details", []
        )
        if processing_details:
            ...
            if "pages" in df.columns:
                chart = (
                    ...
                    .properties(title="Pages per Document", height=300)
                )
    ...
    with bedrock_tab:
        ...
        if model_usage:
            ...
            chart = alt.Chart(model_df).mark_bar().encode(
            ...
            ).properties(title="Input Tokens by Model", height=300) + alt.Chart(
                model_df
            ).mark_bar().encode(
                x=alt.X("Model:N", title="Model"),
                ...
                title="Output Tokens by Model", height=300
            )
            st.altair_chart(chart, use_container_width=True)
        else:
        ...
        if purpose_usage:
            ...
            chart = (
                ...
                .properties(title="Token Usage by Purpose", height=300)
            )
            st.altair_chart(chart, use_container_width=True)
        else:
    ...
    with cost_tab:
        ...
        chart = (
            ...
            .properties(title="Cost Distribution", height=300)
        )
        st.altair_chart(chart, use_container_width=True)
        ...
        if purpose_usage:
                ...
                [
                    {
                        "Purpose": d["Purpose"],
                        "Cost": (
                            d["Input Cost"].replace("$", "")
                            if isinstance(d["Input Cost"], str)
                            else d["Input Cost"]
                        ),
                        "Type": "Input",
                    }
                ...
                + [
                    {
                        "Purpose": d["Purpose"],
                        "Cost": (
                            d["Output Cost"].replace("$", "")
                            if isinstance(d["Output Cost"], str)
                            else d["Output Cost"]
                        ),
                        "Type": "Output",
                    }
            ...
            chart = (
                ...
                .properties(title="Cost by Purpose", height=300)
            )
            st.altair_chart(chart, use_container_width=True)
