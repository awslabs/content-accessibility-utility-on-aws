        ...
        for issue_type, type_issues in issues_by_type.items():
            with st.expander(f"{issue_type} ({len(type_issues)})"):
                for i, issue in enumerate(type_issues):
                    st.markdown(
                        f"### {i+1}. {issue.get('criterion_name', 'No message')}"
                    )
                    ...
                    with col1:
                        ...
                        element_info = (
                            ...
                            or issue.get("element_name")
                            or issue.get("selector", "Unknown")
                        )
                        # Some fields might contain HTML tags - extract just the tag name if present
                        if (
                            element_info
                            and isinstance(element_info, str)
                            and element_info.startswith("<")
                            and ">" in element_info
                        ):
                            element_tag = (
                                element_info.split("<")[1].split(">")[0].split()[0]
                            )
                            element_info = element_tag
                        st.markdown(f"**Element:** {element_info}")
                        page_number = issue.get("location", {}).get(
                            "page_number", "Unknown"
                        )
                    ...
                    with col2:
                        ...
                        color = (
                            "green"
                            if status == "compliant"
                            else "red" if status == "needs_remediation" else "black"
                        )
                        display_text = (
                            "Compliant"
                            if status == "compliant"
    ...
    for issue_type, type_issues in issues_by_type.items():
        with st.expander(f"{issue_type} ({len(type_issues)})"):
            for i, issue in enumerate(type_issues):
                ...
                st.markdown(
                    f"### {i+1}. {issue.get('message', 'No message')} {status_icon}"
                )
                ...
                if remediation_details:
                    ...
                    if remediation_details.get("description"):
                        st.markdown(
                            f"**Description:** {remediation_details.get('description')}"
                        )

                    if remediation_details.get("fix_description"):
                        st.markdown(
                            f"**Fix Applied:** {remediation_details.get('fix_description')}"
                        )

                    if remediation_details.get("failure_reason"):
                        st.markdown(
                            f"**Failure Reason:** {remediation_details.get('failure_reason')}"
                        )

                    # Show before/after HTML if available
                    if remediation_details.get(
                        "before_content"
                    ) and remediation_details.get("after_content"):
