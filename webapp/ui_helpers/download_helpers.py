...
def create_download_button_for_file(
    file_path: str, button_text: str, download_filename: Optional[str] = None
) -> None:
    """
...
def create_download_button_for_directory(
    directory_path: str, button_text: str, download_filename: str
) -> None:
    """
...
def create_download_section_for_remediated_files(remediated_path: str) -> None:
    ...
    if os.path.isdir(remediated_path):
        ...
        html_files = [
            f
            for f in os.listdir(remediated_path)
            if f.lower().endswith(".html")
            and os.path.isfile(os.path.join(remediated_path, f))
        ]
        ...
        if len(html_files) > 1:
            ...
            with col1:
                create_download_button_for_directory(
                    remediated_path, "Download All Files (ZIP)", "remediated_files.zip"
                )

            with col2:
                # Allow downloading individual files
                selected_file = st.selectbox(
                    "Select individual file to download:", html_files
                )
                if selected_file:
        ...
        # Single file
        create_download_button_for_file(
            remediated_path, "Download Remediated HTML", "remediated_document.html"
        )
...
def create_download_section_for_reports(temp_dir: str) -> None:
    ...
    has_reports = (
        ...
        or os.path.exists(json_report_path)
        or os.path.exists(audit_report_path)
    )
    ...
    if has_reports:
        ...
        if os.path.exists(html_report_path):
            create_download_button_for_file(
                html_report_path, "Download HTML Report", "remediation_report.html"
            )
        ...
        if os.path.exists(audit_report_path):
            create_download_button_for_file(
                audit_report_path, "Download Audit Report", "accessibility_audit.json"
            )
