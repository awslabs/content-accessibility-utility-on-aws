...
def process_zip_file(
    zip_path: str, temp_dir: str, config: Config, options: Dict[str, Any]
            ...
            try:
                ...
                if perform_remediation:
                    remediation_options = config.get_remediation_options(
                        fix_images=(
                            options.get("fix_images", True)
                            if "fix_images" in options
                            else True
                        ),
                        fix_headings=(
                            options.get("fix_headings", True)
                            if "fix_headings" in options
                            else True
                        ),
                        fix_links=(
                            options.get("fix_links", True)
                            if "fix_links" in options
                            else True
                        ),
                        severity_threshold=options.get("severity_threshold", "warning"),
                    )
    ...
    combined_audit_result = {
        ...
        "summary": {
            ...
            "info_count": sum(
                1 for issue in combined_issues if issue.get("severity") == "info"
            ),
            "compliant": sum(
                1
    ...
    # Store in results
    results = {"audit_result": combined_audit_result}
        ...
        with st.spinner("Generating remediation report..."):
            try:
                remediation_report_path = os.path.join(
                    temp_dir, "remediation_report.html"
                )
                generate_remediation_report(
