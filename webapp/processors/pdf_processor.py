...
def process_pdf_file(
    pdf_path: str, temp_dir: str, config: Config, options: Dict[str, Any]
    ...
    if perform_remediation:
        remediation_options = config.get_remediation_options(
            fix_images=(
                options.get("fix_images", True) if "fix_images" in options else True
            ),
            fix_headings=(
                options.get("fix_headings", True) if "fix_headings" in options else True
            ),
            fix_links=(
                options.get("fix_links", True) if "fix_links" in options else True
            ),
            severity_threshold=options.get("severity_threshold", "warning"),
        )
