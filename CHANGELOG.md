# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.1.1] - 2026-07-21


### Bug Fixes

- **docs:** Add jekyll-include-cache so the Pages build succeeds ([#10](https://github.com/awslabs/content-accessibility-utility-on-aws/pull/10))



### Documentation

- Add GitHub Pages documentation site (#9) ([#9](https://github.com/awslabs/content-accessibility-utility-on-aws/pull/9))
- Add PyPI version badge to README and Pages navbar (#11) ([#11](https://github.com/awslabs/content-accessibility-utility-on-aws/pull/11))

## [1.1.0] - 2026-07-20


### Features

- Add optional [i18n] content translation package (#8) ([#8](https://github.com/awslabs/content-accessibility-utility-on-aws/pull/8))

## [1.0.0] - 2026-07-20


### Bug Fixes

- **release:** Label changelog with the release version via git-cliff --tag



### Features

- Add browser-backed rendered audit + accessibility agent, and a managed AgentCore deployment (#7) ([#7](https://github.com/awslabs/content-accessibility-utility-on-aws/pull/7))

## [0.7.0] - 2026-07-15


### Bug Fixes

- Added missing build dependency
- Resolved audit issue with blended numbered html pages and non-numbered pages.
- Improved table generation with solid borders
- Update version to 0.6.2 in the accessibility package
- Code scanning alert no. 1: Workflow does not contain permissions (#3) ([#3](https://github.com/awslabs/content-accessibility-utility-on-aws/pull/3))



### CI/CD

- Enhance release workflow with tag validation and TestPyPI support
- Restrict publish workflow to tag pushes only (#5) ([#5](https://github.com/awslabs/content-accessibility-utility-on-aws/pull/5))



### Documentation

- Update streamlit documentation to reference the correct python package



### Features

- **ci:** Separate RC and production release workflows
- Update to latest Bedrock models, add test suite, and WCAG 2.2 target-size support (#6) ([#6](https://github.com/awslabs/content-accessibility-utility-on-aws/pull/6))



### Miscellaneous

- Updated files for local dev
- Empty commit to trigger new commit build
- Add misc/ directory and .DS_Store to gitignore



### Refactoring

- Renamed module

<!-- generated by git-cliff -->
